...
bool Direct3D::Initialize(HWND hwnd)
{
	...
	{
		if (displayModeList[i].Width == ScreenSize::get().width)
		{
			if (displayModeList[i].Height == ScreenSize::get().height)
			{
	...
	error = wcstombs_s((unsigned int*)&stringLength, m_videoCardDescription, 128, adapterDesc.Description, 128);
	if (error != 0)
	{
		return false;
	}
	...
	swapChainDesc.BufferDesc.Width = ScreenSize::get().width;
	swapChainDesc.BufferDesc.Height = ScreenSize::get().height;
	...
	depthBufferDesc.Width = ScreenSize::get().width;
	depthBufferDesc.Height = ScreenSize::get().height;
	...
	viewport.Width = (float)ScreenSize::get().width;
	viewport.Height = (float)ScreenSize::get().height;
	...
	screenAspect = (float)ScreenSize::get().width / (float)ScreenSize::get().height;
	...
	m_orthoMatrix = XMMatrixOrthographicLH((float)ScreenSize::get().width, (float)ScreenSize::get().height, SCREEN_NEAR, SCREEN_DEPTH);
